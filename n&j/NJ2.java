...
public class JerNatProJect extends LinearOpMode {
    ...
    // Rate limiting variable: store the last output power for the PID loop.
    double lastPower = 0;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            // Anti-windup: reset the integral term if error is very small
            if (Math.abs(error) < 5) {
                integral = 0;
            } else {
                integral += error;
            ...
            // Scale down the output when near the target ("slow zone")
            ...
            // -------- Rate Limiting the PID Output -------- //
            // Define the maximum allowed change per iteration.
            double maxDelta = 0.05;  // Tune this value based on your system
            double newPower = pidOutput;
            double delta = newPower - lastPower;
            
            // Clamp the delta value if it exceeds the maximum allowed change.
            if (delta > maxDelta) {
                newPower = lastPower + maxDelta;
            } else if (delta < -maxDelta) {
                newPower = lastPower - maxDelta;
            }
            // Update lastPower for the next iteration
            lastPower = newPower;
            ...
            telemetry.addData("PID Output", newPower);
